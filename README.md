...
shovill \
  ...
  --gsize 5.7M \                                                       # Estimated genome size in base pairs
  ...
  --namefmt "SRR28370701_%05d" \                                        # Naming format for intermediate files
  --cpus 4 \                                                            # Number of CPU threads to use
  --ram 16 \                                                            # RAM allocation in GB
...
```bash
# Set TMPDIR to store temporary/intermediate files generated by Prokka
...
```bash
prokka \
    --evalue 1e-09 \                                         # Set BLAST e-value cutoff for annotation hits
    --coverage 80 \                                           # Minimum coverage (%) for BLAST hits to be considered valid
    --centre ILRI \                                           # Metadata: name of sequencing center or lab
    --cpus 2 \                                                # Number of CPU threads to parallelize annotation
    --prefix SRR28370701 \                                    # Prefix for all output files (GFF, GBK, FAA, etc.)
    --locustag SRR28370701 \                                  # Locus tag prefix for gene identifiers
    --proteins ./databases/prokka/proteins.faa \              # Optional custom protein database for annotation
    --force \                                                 # Overwrite the output directory if it already exists
    --outdir ./results/illumina/klebs/prokka \               # Directory to save all annotated genome files
    ./results/illumina/klebs/shovill/SRR28370701/SRR28370701.fa  # Input assembly file (FASTA)
```
